...
/* Owner always has owner permission and ensure owner appears in members list for display */
...
/* CHANGED EARLIER: only users with permission 'contributor' are allowed to comment */
...
<head>
  ...
  <style>
    ...
    .app-shell { padding:32px; max-width:1650px; margin:auto; }
    .grid { display:grid; grid-template-columns:68% 32%; grid-template-rows:auto 1fr; gap:32px; min-height:calc(100vh - 70px); box-sizing:border-box; }
    .project-card, .task-list, .members-box, .comments-box {
        background:#fff; border-radius:18px; padding:28px; border:1px solid #d5d5d5; box-shadow:0 6px 20px rgba(0,0,0,0.08);
    }
    .btn-row { display:flex; gap:12px; margin-top:12px; flex-wrap:wrap; }
    .btn { padding:10px 14px; border-radius:8px; font-size:14px; font-weight:600; text-decoration:none; cursor:pointer; border:2px solid #1a73e8; color:#1a73e8; background:#fff; }
    .btn-primary { background:#1a73e8; color:#fff; border:none; padding:10px 14px; }
    .btn-danger { background:#d9534f; color:#fff; border:none; padding:10px 12px; border-radius:8px; cursor:pointer; }
    .member { display:flex; gap:18px; align-items:center; padding:16px 0; border-bottom:1px solid #e6e6e6; font-size:17px; }
    .task-item { padding:18px 0; border-bottom:1px solid #ececec; font-size:17px; display:flex; justify-content:space-between; gap:16px; align-items:center; }
    textarea { width:100%; min-height:110px; padding:14px; font-size:16px; border-radius:14px; border:1px solid #ccc; }
    .task-meta { font-size:14px; color:#777; margin-top:6px; }
    form.inline { display:inline-block; margin:0; }
    @media (max-width:1200px) {
      .grid { grid-template-columns:1fr; grid-template-rows:auto auto auto auto; }
      .project-card, .members-box, .task-list, .comments-box { grid-column:1 / -1; max-height:none; }
    }
  </style>
  <script>
  ...
  <div class="app-shell">
    <div class="grid">
      ...
      <div class="project-card">
        ...
        <p style="margin-top:10px; color:#444;">Nội dung: <?=nl2br(htmlspecialchars($project['description'] ?? ''))?></p>
            ...
            <div class="task-item">
              <div style="flex:1;">
                ...
                <?php if (!empty($t['description'])): ?>
                  <div style="margin-top:6px; color:#555;">Mô tả: <?=nl2br(htmlspecialchars($t['description']))?></div>
                <?php endif; ?>
